...
class Chess:
    ...
    def reset(self, EPD='rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -'):
        ...
        self._update_blocks() # Add the 2x2 blocks
    ...
    def display(self):
        ...
        # Iterate through the 8x8 board rows (y) and cells (x)
        for y, row in enumerate(self.board):
            result += f'{8-y}|'
            for x, piece_val in enumerate(row):
                is_blacked_out = False
                # Check if a block has been selected to be blacked out
                if self._blacked_out_coords is not None:
                    # Convert 8x8 coordinates to 4x4 block coordinates
                    current_block_coords = (y // 2, x // 2)
                    if current_block_coords == self._blacked_out_coords:
                        is_blacked_out = True

                # If the current square is in the blackout zone, draw a black square
                if is_blacked_out:
                    result += '■' # The black square symbol
                # Otherwise, use the existing logic
                elif piece_val != 0:
                    result += self.symbols[piece_val]
                else:
                    result += '.'
    ...
    def _update_blocks(self):
        # Initialize a 4x4 list to hold the blocks
        self.board_blocks = [[None for _ in range(4)] for _ in range(4)]

        # Iterate through the 4x4 grid of blocks
        for block_y in range(4):
            for block_x in range(4):
                # Determine the top-left corner of the block on the 8x8 board
                start_y = block_y * 2
                start_x = block_x * 2

                # Extract the 2x2 slice from the main board
                block = [
                    self.board[start_y][start_x : start_x + 2],
                    self.board[start_y + 1][start_x : start_x + 2]
                ]

                # Assign the extracted block to our new structure
                self.board_blocks[block_y][block_x] = block    
                
                

    """
    Specifies the coordinates of the 2x2 block to be blacked out.

    Args:
        block_y (int): The row of the block (0-3).
        block_x (int): The column of the block (0-3).
    """
    
    def blackout_block(self, block_y, block_x):
        if 0 <= block_y <= 3 and 0 <= block_x <= 3:
            self._blacked_out_coords = (block_y, block_x)
        else:
            print("Error: Block coordinates must be between 0 and 3.")
    ...
    def move(self, cur_pos, next_pos):
        ...
        if self.valid_move(cp, np) == True:
            ...
            self._update_blocks() #Updates the 2x2 squares
            return True
        return False
    ...
    """
    def move_blackout_block(self, direction):
        """Moves the blackout block, swapping its contents with the destination block."""
        if self._blacked_out_coords is None:
            print("No block has been blacked out yet.")
            return False

        current_y, current_x = self._blacked_out_coords
        new_y, new_x = current_y, current_x
        ...
        # Check if the new coordinates are within the 4x4 grid
        if 0 <= new_x <= 3 and 0 <= new_y <= 3:
            # The move is valid, so perform the swap
            new_coords = (new_y, new_x)
            self.swap_blocks(self._blacked_out_coords, new_coords)
            
            # Update the position of the visual blacked-out block
            self._blacked_out_coords = new_coords
            self.p_move *= -1 # Switch turns after a block move
            return True
        else:
            # The move is off the board
            print("Block cannot move this way.")
            return False
            
            
            
    def swap_blocks(self, block_a_coords, block_b_coords):
        """Swaps the contents of two 2x2 blocks on the main game board."""
        
        # Calculate the top-left (y, x) of each block on the 8x8 board
        start_y_a, start_x_a = block_a_coords[0] * 2, block_a_coords[1] * 2
        start_y_b, start_x_b = block_b_coords[0] * 2, block_b_coords[1] * 2
        
        # Use a temporary 2x2 list to hold the contents of block B
        temp_block_contents = [[0, 0], [0, 0]]
        
        for i in range(2):
            for j in range(2):
                temp_block_contents[i][j] = self.board[start_y_b + i][start_x_b + j]
                self.board[start_y_b + i][start_x_b + j] = self.board[start_y_a + i][start_x_a + j]
                self.board[start_y_a + i][start_x_a + j] = temp_block_contents[i][j]
        
    
    """
    Input: cur_cord - string representing the current cordinate of the piece
           next_pos - string representing the next cordinate of the piece
        ...
        if cur_pos != None and next_pos != None:
            ...
            if part * self.p_move > 0 and part != 0:
                ...
                if next_pos in v_moves:
                    if self._blacked_out_coords is not None:
                        block_y = next_pos[1] // 2
                        block_x = next_pos[0] // 2
                        if (block_y, block_x) != self._blacked_out_coords:
                            return True
    ...
    class Queen:
        ...
        def movement(game, player, pos, capture=True):
            ...
            for c in range(1, 8):
                # Directions 0-3 are straight, 4-7 are diagonal
                coords = [
                    (pos[0], pos[1] + c),      # Down
                    (pos[0], pos[1] - c),      # Up
                    (pos[0] + c, pos[1]),      # Right
                    (pos[0] - c, pos[1]),      # Left
                    (pos[0] + c, pos[1] + c),  # Down-Right
                    (pos[0] - c, pos[1] + c),  # Down-Left
                    (pos[0] + c, pos[1] - c),  # Up-Right
                    (pos[0] - c, pos[1] - c)   # Up-Left
                ]
                ...
                for i in range(8):
                    if check[i]:
                        ...
                        is_blocked = False
                        if game._blacked_out_coords is not None:
                            if (ty // 2, tx // 2) == game._blacked_out_coords:
                                is_blocked = True

                        if not is_blocked and 0 <= tx <= 7 and 0 <= ty <= 7:
                            target_square = game.board[ty][tx]
                            if target_square == 0:
                                result.append((tx, ty))
                    ...
                    if check[i]:
                        ...
                        if not is_blocked and 0 <= tx <= 7 and 0 <= ty <= 7:
                            ...
                            if target_square == 0:
                                ...
                                result.append((tx, ty))
                                check[i] = False # Block path after capture
                            else:
                                check[i] = False # Blocked by friendly piece
                        else:
                            check[i] = False # Blocked by blackout or edge of board
...
if __name__ == '__main__':
    ...
    #chess_game = Chess()
    block_y=random.randint(1, 2)
    block_x=random.randint(0, 3)
    chess_game.blackout_block(block_y, block_x)
    while True:
        ...
        chess_game.display()
        decision = input('Do you want to move a block or a piece? (Enter b or p)\n')
        if decision == 'p':
            ...
            direction = input('Which block do you want to move: left (l), right (r), up (u), or down (d)?\n')
            if not chess_game.move_blackout_block(direction):
                print("Invalid block move, try again")
